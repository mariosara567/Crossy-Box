...
public class PlayManager : MonoBehaviour
{
  ...
  private void Start()
  {
    ...
    for (int zPos = backViewDistance; zPos < initialGrassCount; zPos++)
    {
      ...
      if(terrain is Grass grass)
        grass.SetTreePercetage(zPos < -1 ? 1 : 0);
    ...
    {
      SpawnRandomTerain(zPos);
    }

    OnUpdateTerrainLimit.Invoke(horizontalSize, travelDistance + backViewDistance);
  }

  private Terrain SpawnRandomTerain(int zPos)
  {
  ...
  public Coin SpawnCoin(int horizontalSize, int zPos, float probability = 0.2f)
  {
    ...
    if (probability >= Random.value)
    {
      ...
      var spawnIndex = Random.Range(0, spawnPosCandidateList.Count);
      return Instantiate(
          coinList[index],
          spawnPosCandidateList[spawnIndex],
          Quaternion.identity);
    }
  ...
  public void UpdateTerrain()
  {
    ...
    SpawnRandomTerain(spawnPosition);
